...
check-device() {
  if [ "$SERIAL" = "unknown" ]; then
	pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Android Package Manager" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz --title="Android Package Manager" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz --title="Android Package Manager" \
...
OPERATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz --title="Android Package Manager" \
       --combobox="Select Operation" Install Uninstall --default Install 2> /dev/null)
if-cancel-exit

if [ "$OPERATION" = "Install" ]; then
	FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz --title="Android Package Manager - Select Apk Files" --multiple --getopenfilename "$DIR" "*.APK *.apk|*.apk" 2> /dev/null)
	if-cancel-exit
	progressbar-start
    ...
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz \
	...
	FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-apk-manager.svgz --title="Android Package Manager" --inputbox="Enter Android applications from textbox separated by whitespace." 2> /dev/null)
	if-cancel-exit
	kill -9 $KdialogPID 2> /dev/null
    progressbar-start
