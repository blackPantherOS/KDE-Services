...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Concatenating $FILENAME" \
...
progressbar-start() {
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-concatenate-media-file.svgz --title="Concatenate Media Files with Same Codec" --progressbar " " 0)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-concatenate-media-file.svgz --title="Concatenate Media Files with Same Codec" \
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-concatenate-media-file.svgz --title="Concatenate Media Files with Same Codec" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT \
...
FILENAME=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-concatenate-media-file.svgz --title="Concatenate Media Files with Same Codec" \
                --inputbox="Enter filename without whitespaces for new concatenated media file" New_Concatenated_Media_File)
if-cancel-exit

DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-concatenate-media-file.svgz --title="Destination Media Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit
