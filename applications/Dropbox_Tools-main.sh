...
check_stderr() {
    if [ "$EXIT" != "0" ]; then
	qdbus $DBUSREF close
	pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz \
                       --title="Dropbox Tools" \
...
copy() {
    if [ -f $HOME/.dropbox/host.db ]; then
        DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-dropbox.svgz --title="Dropbox Tools" --progressbar " " /ProgressDialog)
	...
	ELAPSED_TIME=$((FINAL_TIME-BEGIN_TIME))
        qdbus $DBUSREF close
        elapsed-time
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-dropbox.svgz \
                       --title="Dropbox Tools" \
...
start_service() {
	...
	FINAL_TIME=$(date +%s)
        ...
        qdbus $DBUSREF close
        elapsed-time
	pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-dropbox-on.svgz \
                   --title="Dropbox Tools" \
...
stop_service() {
    ...
    qdbus $DBUSREF close
    elapsed-time
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-dropbox-off.svgz \
                   --title="Dropbox Tools" \
...
Terminal=false
Type=Application
Icon=/usr/share/icons/hicolor/scalable/apps/ks-dropbox.svgz
