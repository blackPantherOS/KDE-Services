...
if-cancel-exit() {
    ...
    if [ "$FORMAT" = "" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Convert Video from ($DIR/)" \
...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Converting video ${i##*/}" \
...
time-position() {
    TIMEPOSITION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --progressbar "                                " $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}s"
    elif [ "$ELAPSED_TIME" -gt "59" ] && [ "$ELAPSED_TIME" -lt "3600" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/60"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}m"
    elif [ "$ELAPSED_TIME" -gt "3599" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/3600"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}h"
    fi
    rm -f $LOG
}
...
MODE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Profile" mobile "Mobile Phones (3GP)" \
     ...
     images2video "Images to Video" multiplexaudio "Multiplex Audio File" customized "Customized (Advanced Users)" --geometry 260x450+$((WIDTH/2-260/2))+$((HEIGHT/2-450/2)) 2> /dev/null)
if-cancel-exit

############################### images2video ###############################
if [ "$MODE" = "images2video" ]; then
	FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Select Image Files In Your Preferred Order" --multiple \
	...
	FRAME_RATE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files|$(ls /tmp/SequentialImage_*|wc -w|sed -e 's/^ *//') selected images" \
                --inputbox="Enter the frame rate of the output video file (for 10 selected image files to 1 fps ~1800)" 1800)
	if-cancel-exit
	FILENAME=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" \
                --inputbox="Enter filename without whitespaces for new video file" New_Video_File)
	if-cancel-exit
	DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Destination Video File" --getexistingdirectory "$DIR" 2> /dev/null)
	if-cancel-exit

	DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --progressbar " " 0)
    ...
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Converting sequential images to $FILENAME.mp4" \
                       --passivepopup="[Canceled]   Check the path and filename not contain whitespaces. Check error log $LOGERROR. Try again"
        mv $LOG $DESTINATION/$LOGERROR
        continue
    fi
    ...
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   $FILENAME.mp4   Elapsed Time: ${ELAPSED_TIME}s"
    elif [ "$ELAPSED_TIME" -gt "59" ] && [ "$ELAPSED_TIME" -lt "3600" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/60"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   $FILENAME.mp4   Elapsed Time: ${ELAPSED_TIME}m"
    elif [ "$ELAPSED_TIME" -gt "3599" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/3600"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --passivepopup="[Finished]   $FILENAME.mp4   Elapsed Time: ${ELAPSED_TIME}h"
    fi
...
if [ "$MODE" = "multiplexaudio" ]; then
	FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Source Video Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT \
	...
	if-cancel-exit
	AUDIO_FILE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Select Audio File" \
	...
	for i in $FILES; do
		if [[ "$(ffprobe $i 2>&1 |grep -o "Stream #0:1")" ]]; then
			pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Multiplexing ${AUDIO_FILE##*/} to ${i##*/}" --passivepopup="[Canceled]  ${i##*/} it already contain audio track. Try again"
			continue
		fi
		logs
		COUNT=$((++COUNT))
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Source Video Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT *.dat *.DV *.dv *.FLV *.flv *.M2V *.m2v *.M4V *.m4v *.MKV *.mkv \
...
if [ "$MODE" = "video2images" ]; then
	IMAGE_FORMAT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Image Format" bmp "BMP" jpg "JPG" pam "PAM" pbm "PBM" pgm "PGM" png "PNG" ppm "PPM" sgi "SGI" tif "TIFF" \
				  --geometry 100x200+$((WIDTH/2-100/2))+$((HEIGHT/2-200/2)) 2> /dev/null)
	if-cancel-exit
	FRAME_RATE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" \
                --inputbox="Enter the frame rate of the input video file")
	if-cancel-exit
	DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Destination Image Files" --getexistingdirectory "$DIR" 2> /dev/null)
	if-cancel-exit
...
DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Destination Video Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit

############################### Mobile ###############################
if [ "$MODE" = "mobile" ]; then
    RESOLUTION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Resolution" 128x96 "SQCIF (128x96)" 176x144 "QCIF (176x144)" 320x240 "QVGA (320x240)" \
				352x288 "CIF (352x288)" --geometry 100x170+$((WIDTH/2-100/2))+$((HEIGHT/2-170/2)) 2> /dev/null)
    if-cancel-exit
...
############################### 4K ###############################
if [ "$MODE" = "4K" ]; then
    CODEC=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Codec" avi "AVI" mpg "MPEG-1" mp4-h.264 \
          "MPEG-4 (H.264)" mp4-h.265 "MPEG-4 (H.265)" webm "WebM" --geometry 100x190+$((WIDTH/2-100/2))+$((HEIGHT/2-190/2)) 2> /dev/null)
    if-cancel-exit
...
############################### same ###############################
if [ "$MODE" = "same" ]; then
    CODEC=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Codec" avi "AVI" flv "FLV" mpg "MPEG-1" mpg2 "MPEG-2" mp4-h.264 \
          "MPEG-4 (H.264)" mp4-h.265 "MPEG-4 (H.265)" webm "WebM" --geometry 100x230+$((WIDTH/2-100/2))+$((HEIGHT/2-230/2)) 2> /dev/null)
    if-cancel-exit
...
if [ "$MODE" = "standards" ]; then
    STD=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" \
        --menu="Choose Standard Video Resolution" vcd "VCD" vcd-700 "VCD (700 MB)" svcd "SVCD" \
        svcd-700 "SVCD (700 MB)" dvd "DVD" dvd-4.7 "DVD (4.7 GB)" dvd-8.0 "DVD (8.0 GB)" --geometry 100x230+$((WIDTH/2-100/2))+$((HEIGHT/2-230/2)) 2> /dev/null)
    if-cancel-exit
    
    if [ "$STD" = "vcd" ] || [ "$STD" = "vcd-700" ];then
        FORMAT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" \
    ...
    if [ "$STD" = "vcd-700" ];then
        time-position
        FILESIZE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert To VCD (700 MB)" \
    ...
    if [ "$STD" = "svcd-700" ];then
        time-position
        FILESIZE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert To SVCD (700 MB)" \
                 --inputbox="Enter the file size limit in bytes, ex: 700 MB = 734003200" 734003200 2> /dev/null)
        if-cancel-exit
    fi
    ...
    if [ "$STD" = "dvd-4.7" ];then
        time-position
        FILESIZE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert To DVD (4.7 GB)" \
    ...
    if [ "$STD" = "dvd-8.0" ];then
        time-position
        FILESIZE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert To DVD (8.0 GB)" \
...
if [ "$MODE" = "web" ]; then
    RESOLUTION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Resolution" 128x96 "SQCIF (128x96)" 176x144 "QCIF (176x144)" 320x240 "QVGA (320x240)" \
				...
				--geometry 220x290+$((WIDTH/2-220/2))+$((HEIGHT/2-290/2)) 2> /dev/null)
    if-cancel-exit
    CODEC=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Codec" flv "FLV" webm "WebM" --geometry 100x100+$((WIDTH/2-100/2))+$((HEIGHT/2-100/2)) 2> /dev/null)
    if-cancel-exit
...
if [ "$MODE" = "customized" ]; then
    RESOLUTION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --inputbox="Enter Custom Video Resolution (ex: 852x480)" 852x480 2> /dev/null)
    if-cancel-exit
    VIDEO_CODEC=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Video Codec" \
			$(ffmpeg -encoders|grep -w 'V\.\.\.\.\.'|grep -v '= Video'|grep -v image|grep -v bitmap|grep -v gif|grep -v jpeg2000|grep -v Uncompressed\
			|awk -F " " '{print $2,$3"_"$4"_"$5"_"$6"_"$7"_"$8"_"$9"_"$10"_"$11"_"$12"_"$13"_"$14"_"$15"_"$16}'|sed 's/_*$//g'|sort -k2) --geometry 600x370+$((WIDTH/2-600/2))+$((HEIGHT/2-370/2)) 2> /dev/null)
    if-cancel-exit
    AUDIO_CODEC=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --menu="Choose Audio Codec" \
				  $(ffmpeg -encoders|grep -w 'A\.\.\.\.\.'|grep -v '= Audio'|grep -v pcm|grep -v comfortnoise\
				  |awk -F " " '{print $2,$3"_"$4"_"$5"_"$6"_"$7"_"$8"_"$9"_"$10"_"$11"_"$12"_"$13"_"$14"_"$15"_"$16}'|sed 's/_*$//g'|sort -k2) --geometry 600x370+$((WIDTH/2-600/2))+$((HEIGHT/2-370/2)) 2> /dev/null)
    if-cancel-exit
 	AUDIO_BITRATE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="Convert Video Files" --inputbox="Enter Custom Audio Bitrate in kbit/s" 320 2> /dev/null)
     if-cancel-exit
