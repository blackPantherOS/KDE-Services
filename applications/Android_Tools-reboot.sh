...
if [ "$(pidof adb)" = "" ]; then
  kdesu -i /usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --noignorebutton -d adb start-server
  if-cancel-exit
fi
...
progressbar-start() {
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --title="Android Reboot Manager" --progressbar " " 0)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --title="Android Reboot Manager" \
...
OPERATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --title="Android Reboot Manager" \
...
if [ "$OPERATION" = "System" ]; then
	...
	if [ "$(adb devices|grep -v List|awk -F" " '{print $2}'|head -n1)" = "device" ] || [ "$(adb devices|grep -v List|awk -F" " '{print $2}'|head -n1)" = "recovery" ]; then
		...
		kdesu -i /usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --noignorebutton -d fastboot reboot
		if-cancel-exit
	fi
    FINAL_TIME=$(date +%s)
	...
	if [ "$(adb devices|grep -v List|awk -F" " '{print $2}'|head -n1)" = "device" ] || [ "$(adb devices|grep -v List|awk -F" " '{print $2}'|head -n1)" = "recovery" ]; then
		...
		kdesu -i /usr/share/icons/hicolor/scalable/apps/ks-android-reboot.svgz --noignorebutton -d fastboot reboot-bootloader
		if-cancel-exit
	fi
	FINAL_TIME=$(date +%s)
	ELAPSED_TIME=$((FINAL_TIME-BEGIN_TIME))
