...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Extracting audio track from ${i##*/} to $FORMAT" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="[Extract|Convert] Audio Track" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="[Extract|Convert] Audio Track" \
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="[Video|Audio] Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT *.dat *.DV *.dv \
...
FORMAT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="[Extract|Convert] Audio Track" \
       --combobox="Choose Audio Encoder" FLAC "FLAC (432Hz)" MP3 "MP3 (432Hz)" OGG "OGG (432Hz)" --default "MP3 (432Hz)" 2> /dev/null)
if-cancel-exit

DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="Destination Audio Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit
    
if [ "$FORMAT" = "MP3 (432Hz)" ]; then
    MODE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio.svgz --title="[Extract|Convert] Audio Track" \
