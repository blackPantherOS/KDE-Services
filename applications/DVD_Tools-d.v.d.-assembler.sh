...
if-dvdauthor-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="D.V.D. Assembler ($DVD_NAME)" \
...
progressbar-start() {
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-optical-video.svgz --title="D.V.D. Assembler" --progressbar "			" /ProcessDialog)
}
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-optical-video.svgz --title="Source Video Files" --multiple \
...
for VIDEO in $FILES; do
    ...
    if [ "$CODEC" != "mpeg2video" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="D.V.D. Assembler" \
                       --passivepopup="[Excluded]   The video file ($VIDEO) isn't MPEG-2 stream." 2> /dev/null
        FILES=$(echo $FILES|sed "s;$VIDEO;;")
        
        if [ "$(echo $FILES)" = "" ]; then
            pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="D.V.D. Assembler" --passivepopup="[Canceled]   Nothing to do." 2> /dev/null
            rm -fr $VIDEOINFO
            exit 1
        fi
...
DVD_NAME=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-optical-video.svgz --title="D.V.D. Assembler" --inputbox="Enter DVD name without whitespaces." 2> /dev/null)
if-cancel-exit

DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-optical-video.svgz --title="Destination DVD" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit
...
if [ "$ELAPSED_TIME" -lt "60" ]; then
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-optical-video.svgz --title="D.V.D. Assembler" \
