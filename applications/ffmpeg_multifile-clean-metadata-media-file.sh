...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Cleaning Metadata from ${i##*/}" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-clean-metadata.svgz --title="Clean Metadata from Media Files" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-clean-metadata.svgz --title="Clean Metadata from Media Files" \
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-clean-metadata.svgz --title="Media Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT \
...
DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-clean-metadata.svgz --title="Destination Media Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit
