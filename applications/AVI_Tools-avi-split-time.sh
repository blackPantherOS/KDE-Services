...
if-avisplit-cancel() {
    if [ "$?" != "0" ]; then
        qdbus $DBUSREF close
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="AVI Split (By Time Range)" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (By Time Range)" --progressbar "\t\t\t\t\t\t\t" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (By Time Range)" \
...
TIMERANGE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (By Time Range)" \
