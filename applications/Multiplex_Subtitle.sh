...
progressbar-start() {
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-multiplexing-subs.svgz --title="Multiplex Subtitle" --progressbar "                                   " /ProcessDialog)
}
...
if [ "$CODEC" != "mpeg2video" ]; then
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Multiplex Subtitle" --passivepopup="[Canceled]   The video file isn't MPEG-2 stream."
    rm -fr $VIDEOINFO
else
    DIR="$(pwd)"

    SUBTITLE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-multiplexing-subs.svgz --title="Text Based Subtitle" \
...
EOF
    ...
    if [ "$EXIT" = "0" ];then
        ...
        if [ "$ELAPSED_TIME" -lt "60" ]; then
            pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-multiplexing-subs.svgz --title="Multiplex Subtitle" \
        ...
        progressbar-close
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Multiplex Subtitle" \
