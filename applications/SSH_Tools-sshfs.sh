...
OPTION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" \
...
if [ "$OPTION" = "Mount Remote Directory" ]; then
    ...
    LOGIN=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" --combobox="Select User" $USER root \
          --default $USER 2> /dev/null)
    if-cancel-exit
    HOST=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" --combobox="Select Hostname or IP Address" $SERVER  \
           --default $(head -n1 ~/.kde-services/machines) 2> /dev/null)
    
    if [ "$?" -gt "0" ]; then
        HOST=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" \
    ...
    if [ "$MOUNTPOINT" = "" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" \
    ...
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sshfs.svgz --title="SSH Tools - Mount point to Remote Directory" --passivepopup="[Finished]" 2> /dev/null
fi
