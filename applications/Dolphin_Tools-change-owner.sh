...
check-stderr() {
    if [ -s "$STDERR" ]; then
        su -c 'pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Change Owner Directory" \
...
progressbar-start() {
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Here" --progressbar "                            " /ProgressDialog)
}
...
rm -f $STDERR
OWNER=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Here" --combobox="Select Owner" $SYSUSERS --default $USER 2> /dev/null)
...
if [ -d "$@" ]; then
    MODE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Here" \
    ...
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Here" --yesnocancel Recursively? 2> /dev/null
    EXIT=$?
    ...
    if [ "$EXIT" = "1" ]; then
        ...
        check-stderr
        progressbar-close
        su -c 'pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Directory" \
...
MODE2=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner Here" --menu="Change Mode Bits (Owner-Group-Others)" 644 "644 (rw-r-r)" \
...
check-stderr
progressbar-close
su -c 'pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-owner.svgz --title="Change Owner File" \
