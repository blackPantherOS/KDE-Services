...
DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-search-name.svgz --title="Modified Files Here" --progressbar "                                       " /ProgressDialog)
qdbus $DBUSREF setLabelText "Searching..."
find $1 -type f -newer "$HOME/.xsession-errors-:0" > $TMP; touch "$HOME/.xsession-errors-:0"
qdbus $DBUSREF close
pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-search-name.svgz --title="Modified Files Here: $(cat $TMP|wc -l) entries" \
