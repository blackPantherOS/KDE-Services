...
elapsed-time() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --passivepopup="[Finished]. Compilation Time: ${ELAPSED_TIME}s" 2> /dev/null
    elif [ "$ELAPSED_TIME" -gt "59" ] && [ "$ELAPSED_TIME" -lt "3600" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/60"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --passivepopup="[Finished]. Compilation Time: ${ELAPSED_TIME}m" 2> /dev/null
    elif [ "$ELAPSED_TIME" -gt "3599" ] && [ "$ELAPSED_TIME" -lt "86400" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/3600"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --passivepopup="[Finished]. Compilation Time: ${ELAPSED_TIME}h" 2> /dev/null
    elif [ "$ELAPSED_TIME" -gt "86399" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/86400"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --passivepopup="[Finished]. Compilation Time: ${ELAPSED_TIME}d" 2> /dev/null
    fi
}
...
before-compile() {
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" \
...
rpmbuild-error() {
    if [ "$?" != "0" ]; then
        ...
        rm -fr /tmp/speak*
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --error="[Error]: See $HOME/rpmbuild/TMP/kernel.err. Try Again." 2> /dev/null
        exit 0
    fi
}
...
install-packages() {
    ...
    play /tmp/speak.wav 2> /dev/null
    rm -fr /tmp/speak*
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --passivepopup="Enter [root] password for bootloader update" 2> /dev/null
    setterm -cursor on
...
KERNELSOURCE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --yesnocancel "Have the kernel source RPM package?" 2> /dev/null)
EXIT=$?
if-cancel-exit
...
if [ "$EXIT" = "0" ]; then
    KERNELSOURCEPATH=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Kernel Source RPM File" \
    ...
    if [ "$EXIT" = "0" ]; then
        if [ "$KERNELVERSION" != "$INTERNETVERSION" ]; then
            pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Kernel Source RPM File" \
...
pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel | B.O.O.=\"$BINOPT\"" \
               --yesnocancel="          Do you want to reconfigure Binary Optimization Option(s) of compilation?               " 2> /dev/null
EXIT="$?"

if [ "$EXIT" = "0" ]; then
    mkdir ~/.kde-services > /dev/null 2>&1
    BINOPT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --combobox="Choose Binary Optimization Option(s)" \
...
pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --yesnocancel "Have the kernel config file?" 2> /dev/null
EXIT=$?
if-cancel-exit
...
if [ "$EXIT" = "0" ]; then
    KERNELCONFIG=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Kernel Config File" --getopenfilename ~/ 2> /dev/null)
    ...
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-kernel-rebuild.svgz --title="Build Custom Kernel" --yesnocancel "Do you want to make changes in the kernel config file?" 2> /dev/null
    EXIT=$?
    if-cancel-exit
