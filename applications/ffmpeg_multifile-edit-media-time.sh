...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Editing time of ${i##*/} from $STIME to $ETIME" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Edit Time from Media Files" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Edit Time from Media Files" \
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Edit Time from Media Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT \
...
DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Destination Video Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit

STIME=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Edit Time from Media Files" \
                --inputbox="Enter start cut time in hh:mm:ss format" 00:00:00)
if-cancel-exit

ETIME=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-media-edit-time.svgz --title="Edit Time from Media Files" \
