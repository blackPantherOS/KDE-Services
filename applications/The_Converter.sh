...
if-convert-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Image Converter" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" --progressbar "                                         " $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}s"
    elif [ "$ELAPSED_TIME" -gt "59" ] && [ "$ELAPSED_TIME" -lt "3600" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/60"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}m"
    elif [ "$ELAPSED_TIME" -gt "3599" ] && [ "$ELAPSED_TIME" -lt "86400" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/3600"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}h"
    elif [ "$ELAPSED_TIME" -gt "86399" ]; then
        ELAPSED_TIME=$(echo "$ELAPSED_TIME/86400"|bc -l|sed 's/...................$//')
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" --passivepopup="[Finished]   ${i##*/}   Elapsed Time: ${ELAPSED_TIME}d"
    fi
}
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Source Image Files" --multiple \
...
DESTINATION=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Destination Image Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit

FORMAT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-image.svgz --title="Image Converter" \
...
if [ "$FORMAT" = "BMP" ]; then
    ...
    for i in $FILES; do
        ...
        qdbusinsert
        DST_FILE="${i%.*}"
        convert $i -quality 100 -transparent white "$DESTINATION/${DST_FILE##*/}.svgz"
        if-convert-cancel
    done
