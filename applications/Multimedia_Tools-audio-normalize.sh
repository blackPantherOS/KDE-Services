...
if-mp3gain-cancel() {
    if [ "$?" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Adjusting volume level of ${i##*/}" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio-normalize.svgz --title="Volume Normalize of MP3 Files" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio-normalize.svgz --title="Volume Normalize of MP3 Files" \
...
FILES=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-audio-normalize.svgz --title="[Video|Audio] Files" --multiple --getopenfilename "$DIR" "*.MP3 *.mp3|*.mp3" 2> /dev/null)
if-cancel-exit
