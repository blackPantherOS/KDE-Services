...
finished() {
    if [ "$?" = "0" ]; then
        ...
        if [ "$ELAPSED_TIME" -lt "60" ]; then
            pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-checksum.svgz --title="Verify $HASH CheckSum" \
        ...
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Verify $HASH CheckSum" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-checksum.svgz --title="Verify $HASH CheckSum" --progressbar "                        " $COUNTFILES)
}
...
for file in $FILE; do
	...
	if [ "$CHECKSUMFILE" != "md5" ] && [ "$CHECKSUMFILE" != "MD5" ] && [ "$CHECKSUMFILE" != "sha1" ] && [ "$CHECKSUMFILE" != "SHA1" ] && \
		...
		pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Verify CheckSum" \
