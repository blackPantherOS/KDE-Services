...
if-avisplit-cancel() {
    if [ "$?" != "0" ]; then
        qdbus $DBUSREF close
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="AVI Split (To Size)" \
...
progressbar-start() {
    ...
    DBUSREF=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (To Size)" --progressbar "\t\t\t\t\t\t\t" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (To Size)" \
...
SIZE=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-video.svgz --title="AVI Split (To Size)" --inputbox="Enter size in MBytes" 2> /dev/null)
if-cancel-exit
progressbar-start
