...
if [ "$UID" != "0" ]; then
    PORT=$(pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-folder-public-web.svgz --title="HTTP Server" \
...
if [ "$PORT" -lt "1" ] || [ "$PORT" -gt "65535" ]; then
    if [ "$UID" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="HTTP Server" --passivepopup="[Error]   Port number $PORT out of range. \
...
if [ "$EXIT" != "0" ]; then
    ...
    if [ "$EXIT" != "0" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="HTTP Server" \
    ...
    if [ "$PORT" != "80" ]; then
        pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-folder-public-web.svgz --title="HTTP Server" --yes-label Background --no-label Stop \
    ...
    pydialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="HTTP Server" --passivepopup="[Error]   Port number $PORT already in use, change it and try again."
fi
